  ...
  .controller('wpStatelessTools', [
    ...
    function ($scope, $http) {
          ...
          function (response) {
            ...
            if (data.success) {
              if (typeof data.data !== 'undefined') {
                ...
                if (
                  $scope.action == 'regenerate_images' &&
                  typeof $scope.progresses.images[1] == 'number'
                ) {
                  ...
                  $scope.action == 'sync_non_images' &&
                  typeof $scope.progresses.other[1] == 'number'
                ) {
                  // Subtracting 1 so user get where from the sync will start instead of where it ended. We will add 1 in class-ajax.php.
        ...
        $http({
          ...
          function (response) {
            ...
            if (data.success) {
              ...
              $scope.status = $scope.getError(
                response,
                stateless_l10n.unable_to_get_images_media_id
              )
              $scope.error = true
            }
            ...
            if (WP_DEBUG) {
              console.log(
                stateless_l10n.wp_stateless_get_images_media_id,
                response
              )
            }
          },
          function (response) {
            ...
            $scope.status = $scope.getError(
              response,
              get_images_media_id + ': ' + stateless_l10n.request_failed
            )
            $scope.isLoading = false
      ...
      $scope.getOtherMedia = function (callback, cont, use_wildcards) {
        ...
        $http({
          ...
          function (response) {
            ...
            if (data.success) {
              if (typeof callback === 'function') {
                if (typeof data.data !== 'undefined') {
                  ...
                  $scope.status = $scope.getError(
                    response,
                    stateless_l10n.ids_are_malformed
                  )
                  $scope.error = true
                }
              }
              ...
              $scope.status = $scope.getError(
                response,
                stateless_l10n.unable_to_get_non_images_media_id
              )
              $scope.error = true
            }
            ...
            if (WP_DEBUG) {
              console.log(
                'WP-Stateless get non Images Media ID:',
                response,
                typeof response.headers === 'function' ? response.headers() : ''
              )
            }
          ...
          function (response) {
            ...
            $scope.status = $scope.getError(
              response,
              stateless_l10n.get_non_images_media_id_request_failed
            )
            $scope.isLoading = false
            if (WP_DEBUG) {
              console.log(
                'WP-Stateless get non Images Media ID: Request failed',
                response,
                typeof response.headers === 'function' ? response.headers() : ''
              )
            }
        ...
        $http({
          ...
          function (response) {
            ...
            if (data.success) {
              ...
              $scope.status = $scope.getError(
                response,
                stateless_l10n.non_libraries_files_are_not_found
              )
            }
            ...
            if (WP_DEBUG) {
              console.log(
                'WP-Stateless get non library files:',
                response,
                typeof response.headers === 'function' ? response.headers() : ''
              )
            }
          ...
          function (response) {
            ...
            $scope.status = $scope.getError(
              response,
              stateless_l10n.get_non_library_files_request_failed
            )
            $scope.isLoading = false

            if (WP_DEBUG) {
              console.log(
                'WP-Stateless get non library files: Request failed',
                response,
                typeof response.headers === 'function' ? response.headers() : ''
              )
            }
      ...
      $scope.syncNonLibraryFiles = function () {
        ...
        $scope.objectsCounter = 0
        $scope.status =
          stateless_l10n.processing_files +
          $scope.objectsTotal +
          stateless_l10n._total___
      ...
      $scope.syncFiles = function () {
        ...
        if ($scope.objectIDs.length) {
          ...
          $scope.chunkIDs = array_bulk($scope.objectIDs, $scope.bulk_size)
          for (var i in $scope.chunkIDs) {
            $scope.syncSingleFile(
              $scope.chunkIDs[i].shift(),
              $scope.use_wildcards,
              i
            )
          }
      ...
      $scope.regenerateImages = function () {
        ...
        $scope.objectsCounter = 0
        $scope.status =
          stateless_l10n.processing_images +
          $scope.objectsTotal +
          stateless_l10n._total___
      ...
      $scope.regenerateSingle = function (id, use_wildcards, chunk_id) {
        $http({
          ...
          function (response) {
            ...
            $scope.log.push({
              message:
                data.data || stateless_l10n.regenerate_single_image_failed,
            })

            jQuery('#regenthumbs-bar').progressbar(
              'value',
              (++$scope.objectsCounter / $scope.objectsTotal) * 100
            )
            jQuery('#regenthumbs-bar-percent').html(
            ...
            if (
              ...
              $scope.status = $scope.getError(
                response,
                stateless_l10n.regenerate_single_image_failed
              )
              // $scope.isRunning = false;
            }
            ...
            if (WP_DEBUG) {
              console.log(
                'WP-Stateless regenerate single image:',
                response,
                typeof response.headers === 'function' ? response.headers() : ''
              )
            }
          ...
          function (response) {
            ...
            $scope.status = $scope.getError(
              response,
              stateless_l10n.regenerate_single_image_request_failed
            )
            $scope.isRunning = false
            if (WP_DEBUG) {
              console.log(
                'WP-Stateless regenerate single image: Request failed',
                response,
                typeof response.headers === 'function' ? response.headers() : ''
              )
            }
      ...
      $scope.syncSingleFile = function (id, use_wildcards, chunk_id) {
        $http({
          ...
          function (response) {
            var data = response.data || {}
            $scope.log.push({
              message: $scope.getError(
                response,
                stateless_l10n.sync_single_file_failed
              ),
            })
          ...
          function (response) {
            ...
            $scope.status = $scope.getError(
              response,
              stateless_l10n.sync_single_file_request_failed
            )
            $scope.isRunning = false

            if (WP_DEBUG) {
              console.log(
                'WP-Stateless sync single file: Request failed',
                response,
                typeof response.headers === 'function' ? response.headers() : ''
              )
            }
        ...
        $http({
          ...
          function (response) {
            ...
            $scope.log.push({
              message: $scope.getError(
                response,
                stateless_l10n.failed_to_sync + id
              ),
            })
            ...
            if (
              ...
              $scope.status = $scope.getError(
                response,
                stateless_l10n.sync_non_library_file_failed
              )
              // $scope.isRunning = false;
            }
            ...
            if ('undefined' !== typeof chunk_id) {
              if ($scope.chunkIDs[chunk_id].length && $scope.continue) {
                $scope.syncSingleNonLibraryFile(
                  $scope.chunkIDs[chunk_id].shift(),
                  chunk_id
                )
              } else {
            ...
            if (WP_DEBUG) {
              console.log(
                'WP-Stateless sync non library file:',
                response,
                typeof response.headers === 'function' ? response.headers() : ''
              )
            }
          ...
          function (response) {
            ...
            $scope.status = $scope.getError(
              response,
              stateless_l10n.sync_non_library_file_request_failed
            )
            $scope.isRunning = false

            if (WP_DEBUG) {
              console.log(
                'WP-Stateless sync non library file: Request failed',
                response,
                typeof response.headers === 'function' ? response.headers() : {}
              )
            }
