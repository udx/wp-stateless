    ...
    function ud_check_stateless_media()
    {
        ...
        try {
            ...
            if (!file_exists($file)) {
                throw new Exception(
                    __(
                        "Distributive is broken. composer.json is missed. Try to remove and upload plugin again.",
                        "stateless-media"
                    )
                );
            }
            ...
            if (!empty($data["require"]["php"])) {
                ...
                if (!empty($matches[1]) && !empty($matches[2])) {
                    if (
                        !version_compare(PHP_VERSION, $matches[2], $matches[1])
                    ) {
                        throw new Exception(
                            sprintf(
                                __(
                                    "Plugin requires PHP %s or higher. Your current PHP version is %s",
                                    "stateless-media"
                                ),
                                $matches[2],
                                PHP_VERSION
            ...
            if (file_exists(dirname(__FILE__) . "/vendor/autoload.php")) {
                ...
                throw new Exception(
                    sprintf(
                        __(
                            "Distributive is broken. %s file is missed. Try to remove and upload plugin again.",
                            "stateless-media"
                        ),
            ...
            if (!class_exists("\wpCloud\StatelessMedia\Bootstrap")) {
                throw new Exception(
                    __(
                        "Distributive is broken. Plugin loader is not available. Try to remove and upload plugin again.",
                        "stateless-media"
                    )
                );
            }
            ...
            require_once dirname(__FILE__) .
                "/lib/meta-box-tabs/meta-box-tabs.php";
        } catch (Exception $e) {
...
if (!function_exists("ud_stateless_media_message")) {
    ...
    function ud_stateless_media_message()
    {
        ...
        if (!empty($_ud_stateless_media_error)) {
            $message = sprintf(
                __(
                    "<p><b>%s</b> can not be initialized. %s</p>",
                    "stateless-media"
                ),
                "Stateless Media",
                $_ud_stateless_media_error
            );
            echo '<div class="error fade" style="padding:11px;">' .
                $message .
                "</div>";
        }
    }
